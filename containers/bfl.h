...
//  gettimeofday(&start_at, 0);
  ...
  if (!file)
  {
      cerr << endl << "Error: cannot open BFL file \"" << filename << "\"" << endl << endl;
      exit(1);
  }
...
size_t getSize()
{
    ...
    for (node &n: nodes)
//        // |N_O| + |N_I| + |L_in| + |L_out| + |L_interval|
//        size += n.N_O_SZ*sizeof(int) + n.N_I_SZ*sizeof(int) + K*sizeof(int) + K*sizeof(int) + 2*sizeof(int);
        // |L_in| + |L_out| + |L_interval|
        size += K*sizeof(int) + K*sizeof(int) + 2*sizeof(int);
